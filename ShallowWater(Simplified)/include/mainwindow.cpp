...
//                TempGrid[i][j] = TempH;
//
//                Energy += ( pow((Grid[i][j].x - TempGrid[i][j].x) / Test -> GetTimeStep(), 2.0) +
...
//
//        for (unsigned int i = 0; i < PointNum; i++) {
//            Energy += ( pow((Grid[i][0].x - TempGrid[i][0].x) / Test -> GetTimeStep(), 1.0) +
//                        pow(TempGrid[i][0].y / TempGrid[i][0].x, 2.0) +
//                        pow(TempGrid[i][0].z / TempGrid[i][0].x, 2.0));
//        }
//        for (unsigned int i = 1; i < PointNum - 1; i++) {
//            Energy += ( pow((Grid[0][i].x - TempGrid[0][i].x / Test -> GetTimeStep()), 2.0) +
...
void Solver::InitGrid(double ExcitationP, double VXP, double VYP, int PointsNumP) {
    for (int i = 0; i < PointsNumP; i++) {
        ...
        for (int j = 0; j < PointsNumP; j++) {
            Grid.back().emplace_back(dVectorND <double>({ExcitationP, VXP * ExcitationP, VYP * ExcitationP}));
        }
    }
}